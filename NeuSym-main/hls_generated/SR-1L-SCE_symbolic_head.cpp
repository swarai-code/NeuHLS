...
//   All 7 inputs read into local scalars before arithmetic begins.
...
// ← ARRAY_PARTITION pragma removed
    ...
    feat_t x121 = sparse_x[0];
    feat_t x24  = sparse_x[1];
    feat_t x0   = sparse_x[2];
    feat_t x63  = sparse_x[3];
    feat_t x47  = sparse_x[4];
    feat_t x66  = sparse_x[5];
    feat_t x38  = sparse_x[6];

    // Constant folding
    // -4.668614 absorbed as compile-time bias — no runtime adder.
    const acc_t BIAS = (acc_t)(-4.668614f);

    // x38 term (separate — different coefficient)
    acc_t term_x38 = scale_2p856(x38);   // 2.8560774 * x38

    // Balanced binary addition tree for the scaled group
    // Inner sum: x121 + x24 - x0 + x63 - x47 - x66 + BIAS
    // Positive: x121, x24, x63
    // Negative: x0, x47, x66
    // Plus: BIAS

    // Level 1 — 3 parallel ops
    acc_t l1_a = (acc_t)x121 + (acc_t)x24;   // x121 + x24
    acc_t l1_b = (acc_t)x63  - (acc_t)x0;    // x63  - x0
    acc_t l1_c = (acc_t)x47  + (acc_t)x66;   // x47  + x66 (both neg, subtract below)
